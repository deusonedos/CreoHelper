...
export function formatHelp(botUsername?: string): string {
  ...
  return [
    ...
    `- Текст: <code>/find запрос</code> (в группе)`,
    "- Голос: просто отправь voice note (бот обработает только от разрешённых пользователей)",
    "",
    "<b>Пример</b>",
    "<code>/find Подбери 5 ключевых слов для приложения фоторедактора и найди TikTok видео за последний месяц с самым большим количеством просмотров</code>",
    "",
    "<b>Подсказка</b>",
    `- Если бот не видит голосовые в группе — отключи privacy mode у ${mention} в BotFather.`,
  ].join("\n");
}
